...
class HistoriqueController extends Controller
{
	...
	public function getetaterreurdefalcation()
    {
		$list = DB::table('tracecomptes')
                    ->whereIn('tracecomptes.moiscalculer', ['08-2024'])
                    ->orderBy('tracecomptes.Commercial', 'desc')
                    ->get();
		...
		foreach ($list as $value){
			...
			if($comp['compte'] == 0 && $comp['retenue'] != 0){ 
				$retanc = $comp['compteNetapayerMoisCalculer'];
				$i++;
				DB::table("compteagents")->where('Agent', $comp['Agent'])->update([
                            //"compteBloquer" => intval( $retanc )
                        ]); 
			}
		}
		dd($i);
		...
		dd("Bon");
        $titre = "ETAT DES DEFALCATIONS RESTANTE A PRELEVE";

            $list = DB::table('tracecomptes')
                    ->join('commerciauxes', 'commerciauxes.codeCom', '=', 'tracecomptes.Commercial')
                    ->whereIn('tracecomptes.moiscalculer', ['05-2024', '06-2024', '07-2024', '08-2024' ])
                    //->where('tracecomptes.contenu', 'like', '%"compteNetapayerMoisCalculer":-%')
                    ->orderBy('tracecomptes.Commercial', 'desc')
                    ->get();
		...
		$i = 1;
            ...
            $path = "Historique_Globale".date('dmYhis').".xlsx";
        
            $tabl[0]["code"] = " Code Apporteur ";
            $tabl[0]["nom"] = " Nom Apporteur ";
            $tabl[0]["prenom"] = " Prénoms Apporteur";
            //$tabl[0]["niv"] = " Niveau Apporteur";
            $tabl[0]["gains"] = " Commission ";
			$tabl[0]["prel"] = " Défalcation ";
			$tabl[0]["solde"] = " Solde ";
			$tabl[0]["mois"] = " MOIS ";
            /*$tabl[0]["enc"] = " Encadrement ";
            $tabl[0]["fixe"] = " Fixe ";
            $tabl[0]["bonus"] = " Bonus";
            $tabl[0]["brut"] = " Montant Brute (A)";
            $tabl[0]["aib"] = " Montant AIB (B) ";
            $tabl[0]["restantaib"] = " (A) - (B) = (C) ";
            $tabl[0]["restantant"] = " Solde antérieur (D) ";
            $tabl[0]["soldedispo"] = " Solde (C) + (D) = (E) ";
            $tabl[0]["prel"] = " Défalcation (F) ";
            $tabl[0]["restantprel"] = " Solde (E) - (F) = (G)";
            $tabl[0]["ecart"] = " Ecart Observe "; */
            ...
            foreach ($list as $value){
				...
				$restaib = ($brut - $comp['aibMoisCalculer']);
				$soldedispo = ($restaib + ($comp['compteBloquerBackup']));
                $tabl[$i]["gains"] = $soldedispo;
                ...
                $tabl[$i]["prel"] = (isset($comp['traceretenue']) && $comp['traceretenue'] != 0) ? $comp['traceretenue'] : $comp['retenue'];
                $restprel = ($soldedispo - $comp['retenue']);
                //$tabl[$i]["solde"] = $restprel;
				$tabl[$i]["soldenette"] = $comp['compte'];
				$tabl[$i]["mois"] = $value->moiscalculer;
				/*
				$libanc = DB::table("compteagents")->where('Agent', $value->Commercial)->first()->libretenue;
				$retanc = DB::table("compteagents")->where('Agent', $value->Commercial)->first()->retenue;
				DB::table("compteagents")->where('Agent', $value->Commercial)->update([
                            "retenue" => intval( ($retanc + ($comp['compteNetapayerMoisCalculer'] * -1))),
                            "libretenue" => 'Restant '.$comp['libretenue'].' '.$libanc
                        ]); 
                $tabl[$i]["ecart"] = $comp['compteNetapayerMoisCalculer'];
				*/
				$i++;
            }
	...
	public function gethistcommission(){
		...
		return view("historique.listcommission", compact('list'));
	}
	
	public function sethistcommissionExceltraitementerreur()
    {
        ...
        $list = $list->where('tracecomptes.moiscalculer', "12-2023")->get();
        ...
        if ($request->codeApporteur == null && $mois == "" ) {
            ...
            foreach ($list as $value){
                ...
                $restprel = ($soldedispo - $comp['retenue']) > 0 ? ($soldedispo - $comp['retenue']) : 0;
            ...
            $tabl[$i]["naf"] = number_format($somnaf, 0, '.', ' ')." CFA";
	...
	public function gethistcommissiondetail(){
	   ...
	   return view("historique.listdetailcom", compact('list', 'taux', 'nomapp', 'mois', 'niveau', 'detailCom', 'comp', 'comm'));
	}
    ...
    public function sethistcommission(Request $request)
    {
        ...
        if(request('excel') == "EXCEL"){
            ...
            $pdf->Cell(22,6,'Code Apporteur', 1, 0, 'C', 1);
            $pdf->Cell(45,6,iconv('utf-8','cp1252','Nom et Prénoms Apporteur'), 1, 0, 'C', 1);
            $pdf->Cell(20,6,'IFU', 1, 0, 'C', 1);
            $pdf->Cell(22,6,'Gains', 1, 0, 'C', 1);
            $pdf->Cell(15,6,'Taux AIB', 1, 0, 'C', 1);
            $pdf->Cell(22,6,'AIB', 1, 0, 'C', 1);
            $pdf->Cell(22,6,iconv('utf-8','cp1252','Avance Com. Remb.'), 1, 0, 'C', 1);
            $pdf->Cell(22,6,iconv('utf-8','cp1252','Prelèvement'), 1, 0, 'C', 1);
            $pdf->Cell(18,6,iconv('utf-8','cp1252','Carec'), 1, 0, 'C', 1);
            $pdf->Cell(18,6,iconv('utf-8','cp1252','Amical'), 1, 0, 'C', 1);
            $pdf->Cell(18,6,iconv('utf-8','cp1252','Naf'), 1, 0, 'C', 1);
            $pdf->Cell(25,6,'Commission Nette', 1, 0, 'C', 1);
            $pdf->Cell(18,6,iconv('utf-8','cp1252','Période'), 1, 0, 'C', 1);
            $som = 0;
            $somgain = 0;
            $somaib = 0;
            $somavan = 0;
            ...
            foreach ($list as $value){ 
                $comp = InterfaceServiceProvider::RecupCompteAncien($value->Commercial, $value->moiscalculer);
                $pdf->Ln(6);
                $pdf->Cell(22,6,iconv('utf-8','cp1252', $value->Commercial ), 1, 0, 'C');
                ...
                if($comp['compteBloquerBackup'] < 0){
                    $pdf->Cell(22,6,iconv('utf-8','cp1252', number_format(($comp['bonus'] + $comp['fixe'] + 
                ...
                if($net_temp == 0)
                    $pdf->Cell(15,6,iconv('utf-8','cp1252', "" ), 1, 0, 'C');
                else
                    $pdf->Cell(15,6,iconv('utf-8','cp1252', round(($comp['aibMoisCalculer'] / $net_temp) * 100). ' %' ), 1, 0, 'C');
                $pdf->Cell(22,6,$comp['aibMoisCalculer'].' CFA', 1, 0, 'C');
                $pdf->Cell(22,6, number_format($comp['recentrembourcer'] , 0, '.', ' ').' CFA', 1, 0, 'C');
                $pdf->Cell(22,6, number_format($comp['retenue'], 0, '.', ' ')." CFA", 1, 0, 'C');
                
                if(isset($comp['traceCarec'])){
                    $pdf->Cell(18,6, number_format($comp['traceCarec'], 0, '.', ' ')." CFA", 1, 0, 'C');
                    //$tabl[$i]["carec"] = $comp['traceCarec'];
                    $somcarec += $comp['traceCarec'];
                }else{
                    $pdf->Cell(18,6, number_format(0, 0, '.', ' ')." CFA", 1, 0, 'C');
                    //$tabl[$i]["carec"] = 0;
                    $somcarec += 0;
                }
                if(isset($comp['tracesAmical'])){
                    $pdf->Cell(18,6, number_format($comp['tracesAmical'], 0, '.', ' ')." CFA", 1, 0, 'C');
                    //$tabl[$i]["amical"] = $comp['tracesAmical'];
                ...
                if(isset($comp['tracenaf'])){
                    $pdf->Cell(18,6, number_format($comp['tracenaf'], 0, '.', ' ')." CFA", 1, 0, 'C');
                    //$tabl[$i]["naf"] = $comp['tracenaf'];
                    $somnaf += $comp['tracenaf'];
                }else{
                    $pdf->Cell(18,6, number_format(0, 0, '.', ' ')." CFA", 1, 0, 'C');
                    //$tabl[$i]["naf"] = 0;
                    $somnaf += 0;
                ...
                $pdf->Cell(25,6, number_format($comp['compte'], 0, '.', ' ').' CFA', 1, 0, 'C');
                
                $som += $comp['compte'];
                $somaib += $comp['aibMoisCalculer'];
                $somavan += $comp['recentrembourcer'];
                $somprel += $comp['retenue'];
                $pdf->Cell(18,6, $value->moiscalculer, 1, 0, 'C');
            ...
            $pdf->Cell(22,6,number_format($somgain, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(15,6,'', 1, 0, 'C', 1);
            $pdf->Cell(22,6,number_format($somaib, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(22,6,number_format($somavan, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(22,6,number_format($somprel, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(18,6,number_format($somcarec, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(18,6,number_format($somamical, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(18,6,number_format($somnaf, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(25,6, number_format($som, 0, '.', ' ').' CFA', 1, 0, 'C', 1);
            $pdf->Cell(18,6,iconv('utf-8','cp1252', $mois ), 1, 0, 'C', 1);
    ...
    public function sethistcommissionExcel($request)
    {
        ...
        if ($request->codeApporteur == null && $mois == "" ) {
            ...
            // Création du fichier Excel
            $name = "Historique_Globale_".date('dmYhis');
            ...
            foreach ($list as $value){
                ...
                $tabl[$i]["peri"] = $value->moiscalculer;
				$tabl[$i]["nafreeel"] = $comp['naf'];
				$tabl[$i]["nafimpaye"] = $comp['impayernaf'];
    ...
    public function listCm(){
        $list = DB::table('signataires')->get();
        
		return view("controle.control", compact('list'));
    }
