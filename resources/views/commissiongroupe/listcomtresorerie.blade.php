		...
		<div class="box-content bordered info js__card">
			...
			<div class="js__card_content">
				...
				<div class="row small-spacing">
					<!------------------------------------------>
					<form class="form-horizontal" action="{{route('GCTG')}}" method="GET" id="recherche">
                    ...
                    <!------------------------------------------->
					<form class="form-horizontal" method="post" action="{{ route('SCTG') }}" id="regle">
										<input type="hidden" name="_token" value="{{ csrf_token() }}" />
										...
										<div class="col-sm-6 col-xs-12 col-lg-6" style="margin-top:-70px">
											...
											<div class="col-sm-5">
												<select type="number" class="form-control" id="inp-type-3" name="reglement" required>
													@if($reglement != null && $reglement=="" && $reglement != "all")
				    ...
				    <div id="ref">
						Ref  <b style="color:#001e60">SP <img src="{{ $signaturesp }}" id="signataire"></b>/<b style="color:#001e60">CSP <img src="{{ $signaturecsp }}" id="signataire"> </b>/<b style="color:#001e60">DT  <img src="{{ $signaturedt }}" id="signataire"> </b>/ <b style="color:#001e60">DG <img src="{{ $signaturedg }}" id="signataire"> </b>/ <b style="color:#001e60">CDAF <img src="{{ $signaturecdaf }}" id="signataire"> </b> / <b style="color:#001e60">T </b>/{{$sigle}}
					</div>
					...
					@endif
                    @if(in_array("reglement_com", session("auto_action")))
					<a class="btn-warning dropdown-item " id="valid" href="{{ route('GRTG') }}"
					   >Règlement <i class="fa fa-bars" aria-hidden="true"></i> </a>
					@endif
					@if(in_array("export_pdf_com", session("auto_action")))
					...
					@endif
					@if(in_array("export_excel_com", session("auto_action")))
					...
					<div class="table-responsive" data-pattern="priority-columns">
						<table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
							...
							<tr>
								...
								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ $apayer }} CFA </th>
							...
							<tbody>
								...
								<tr>
									...
									<td style="vertical-align:middle; text-align: center;">{{$comp->ModePayement}} {{$comp->libCompte}}</td>
									<td style="vertical-align:middle; text-align: center;">{{$comp->numCompte}}</td>
									<td style="vertical-align:middle; text-align: center;">{{$comp->compteGroupe}} CFA</td>
	    ...
	    $(function () {
            ...
            if(id.substr(-4, 4) == "reje"){
                ...
                div.innerHTML = '<div class="modal-header">' +
                    ...
                    '<form class="form-horizontal" method="post" action="{{ route("RCTG") }}">' +
                        '<input type="hidden" name="_token" value="{{ csrf_token() }}" />' +
            ...
            if(id.substr(-4, 4) == "conf"){
                div.innerHTML = '<div class="modal-header">' +
                    ...
                    '</div>    </div>' +
                    '<div class="modal-footer">   ' +
                    '<button type="button" class="btn btn-warning btn-sm waves-effect waves-light" style="float:left; color:white;"><a href="{{ route("listCommConfirmG") }}">' +
