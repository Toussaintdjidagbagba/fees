					...
					<form class="form-horizontal" method="post" action="{{ route('ModifS') }}" >
							...
							<div class="form-group"> 
								<div class="col-sm-6">
									...
									<div class="col-sm-12">
										<input type="hidden" name="id" value="{{ $info->idSchema }}" />
								...
								<div class="col-sm-12">
									<input type="number" step="0.00001" class="form-control" id="inp-type-1" value="{{$info->tauxAIB}}" name="tauxaib">
								</div>
							    </div>			
							</div>
							<div class="form-group">
								...
								<div class="col-sm-12">
									<input type="number" step="0.00001" class="form-control" id="inp-type-1" value="{{$info->tauxNonAIB}}" name="tauxnonaib">
								</div>
							    </div>		
							  <div class="col-sm-6">
								<label for="inp-type-2" style="vertical-align:middle;" class="col-sm-12 ">Produit :</label>
								<div class="col-sm-12">
									<select type="text" class="form-control" id="inp-type-1" name="prod">
										<option value="{{$info->Produit}}">{{ App\Providers\InterfaceServiceProvider::libprod($info->Produit)}}</option>
										@foreach($prodall as $pro)
										@if($info->Produit != $pro->codeProduit)
