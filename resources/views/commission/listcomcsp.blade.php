	...
	<div class="col-lg-12 col-md-12 col-xs-12"> 
		...
		<div class="box-content bordered info js__card"> 
			...
			<h4 class="box-title with-control">
				...
				Liste Commissions {{utf8_encode(strtoupper(strftime('%B %Y'))) }} NSIA ( {{$libelleRole}} ) : 
			...
			<div class="js__card_content">
				...
				<div class="row small-spacing">
					<div id="ref">
						Ref <b style="color:#001e60">SP <img src="{{ $signaturesp }}" id="signataire"></b>/<b style="color:#001e60">CSP</b>/{{$sigle}}
					</div> 
					@if(in_array("confirm_csp_com", session("auto_action")))
					     @if(isset($list) && count($list) != 0)
					        <a class="btn-warning dropdown-item autrecom" id="valid" href="#autrCom"
					            data-toggle="modal" data-id="{{$libelleRole}}conf" title="">Valider <i class="fa fa-check-circle fa-spin" aria-hidden="true"></i> </a>
					     @else
					        <a class="btn-warning dropdown-item autrecom" id="valid" href="#autrCom"
					            data-toggle="modal" data-id="{{$libelleRole}}conf" title="">Valider <i class="fa fa-check" aria-hidden="true"></i> </a>
					     @endif
					@endif 
					@if(in_array("rejet_com", session("auto_action")))
					<a class="btn-danger dropdown-item autrecom" id="valid" href="#autrCom"
					   data-toggle="modal" data-id="{{$libelleRole}}reje" title="">Rejeter <i class="fa fa-ban" aria-hidden="true"></i> </a>
					@endif 
					<a class="btn-primary dropdown-item" style="float: right; margin-right: 10px" href="{{ route('GEDREXCEL') }}">Exportation en excel </a>
					<!--a class="btn-primary dropdown-item" style="float: right; margin-right: 10px" href="{{ route('GEDREXCEL') }}">Etat détail </a-->
					<!------------------------------------------>
					<!--form class="form-horizontal" action="{{route('GCCSP')}}" method="GET" id="recherche">
					...
					<div class="table-responsive" data-pattern="priority-columns">
						<table id="tech-companies-1" class="table table-small-font table-bordered table-striped">
							<?php
								...
								if (isset($list)) {
									foreach($list as $com){
									    ...
									    $combrute = $comp->compteMoisCalculer + $comp->compteEncadrementMoisCalculer + $comp->AutreCommissionMoisCalculer + $comp->bonus + $comp->fixe + $comp->compteBloquerBackup;
										$nette += $combrute;
										$aib += $comp->aibMoisCalculer;
										$retenue += $comp->retenue;
										$naf += $comp->tracenaf;
							...
							<tr>
								...
								<th colspan="11" data-priority="1" style="vertical-align:middle; text-align: center;">Montant Total</th>
							</tr>
							<tr>
								
								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($nette, 0, '.', ' ') }} CFA </th>
								
								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($aib, 0, '.', ' ') }} CFA </th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($retenue, 0, '.', ' ') }} CFA </th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($naf, 0, '.', ' ') }} CFA </th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($avance, 0, '.', ' ') }} CFA</th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($rembource, 0, '.', ' ') }} CFA</th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($resteapayer, 0, '.', ' ') }} CFA</th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($carec, 0, '.', ' ') }} CFA </th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($amical, 0, '.', ' ') }} CFA </th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($bloque, 0, '.', ' ') }} CFA</th>

								<th data-priority="1" style="vertical-align:middle;text-align: center;"> {{ number_format($apayer, 0, '.', ' ') }} CFA</th>
							...
							<tr>
									...
									<th data-priority="1" style="vertical-align:middle;text-align: center;">Défalcation</th>

									<th data-priority="1" style="vertical-align:middle;text-align: center;">Naf</th>
									...
									<th data-priority="1" style="vertical-align:middle;text-align: center;">Bloquer</th>
							...
							<tbody>
								...
								<tr>
									...
									<td style="vertical-align:middle; text-align: center;">{{ App\Providers\InterfaceServiceProvider::infomanageur($com->Commercial) }}</td>
									<!--td style="vertical-align:middle; text-align: center;">{{$comp->libCompte}}</td>
									<td style="vertical-align:middle; text-align: center;">{{$comp->numCompte}}</td-->
									@php 
										$combrute = $comp->compteMoisCalculer + $comp->compteEncadrementMoisCalculer + $comp->AutreCommissionMoisCalculer + $comp->bonus + $comp->fixe + $comp->compteBloquerBackup;
									@endphp
									<td style="vertical-align:middle; text-align: center;">{{number_format($combrute, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->aibMoisCalculer, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->retenue, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->tracenaf, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->avancesancien, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->recentrembourcer, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->avances, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->traceCarec, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->tracesAmical, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->compteBloquer, 0, '.', ' ')}} CFA</td>
									<td style="vertical-align:middle; text-align: center;">{{number_format($comp->compte, 0, '.', ' ')}} CFA</td>

									<td style="vertical-align:middle; text-align: center;">
									    <form action="detailcommission-{{ $com->Commercial }}">
								...
								@empty
								<tr>
									<td colspan="13"><center>Commission indisponible pour ce mois!!! </center></td>
								</tr>
								@endforelse
							</tbody>
						</table>
	    ...
	    $(function () {
            ...
            if(id.substr(-4, 4) == "conf"){
                ...
                div.innerHTML = '<div class="modal-header">' +
                    ...
                    ' <div class="form-group"> ' +
                        '<div class="col-sm-12"> ' +
                        ' <label for="comment" class="col-sm-12">Commentaire : </label>' +
                        ' <div class="col-sm-12"><input type="text" class="form-control" id="comment" name="comment">' +
                        '</div> ' +
                        '</div>' +
                        '    </div>  </div>' +
                    '<div class="modal-footer">   ' +
                    '<a href="{{ route("ConfirmCSP") }}">' +
                    '<button type="button" class="btn btn-warning btn-sm waves-effect waves-light" style="float:left; color:white;">' +
					
					'Confirmer <i class="ico fa fa-check"></i></button> </a> ' +
            ...
            if(id.substr(-4, 4) == "reje"){
                ...
                div.innerHTML = '<div class="modal-header">' +
                    ...
                    '</div><div class="modal-body">' +
                    '<form class="form-horizontal" method="post" action="{{ route("RCCSP") }}">' +
                        '<input type="hidden" name="_token" value="{{ csrf_token() }}" />' +
